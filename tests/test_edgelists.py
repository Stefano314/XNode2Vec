...
@given(arrays(np.int8, array_shapes(min_dims=2, max_dims=2, min_side=2, max_side=4)))
def test_complete_dimension(array):
    """
    Description
    -----------
    ...
    dataset squared.
    """
    assert len(complete_edgelist(array).index) == array.shape[0]**2

@given(arrays(np.int8, array_shapes(min_dims=2, max_dims=2, min_side=2, max_side=4)))
def test_stellar_dimension(array):
    """
    Description
    -----------
    ...
    dataset.
    """
    assert len(stellar_edgelist(array).index) == array[:,0].size
...
@given(arrays(np.int8, array_shapes(min_dims=2, max_dims=2, min_side=2, max_side=4)))
def test_complete_checktypes(array):
    """
    Description
    -----------
    Test of complete_edgelist() function.
